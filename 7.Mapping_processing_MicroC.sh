...
mkdir -p MicroC_output/mapping_stats
mkdir -p MicroC_output/sam
mkdir -p MicroC_output/pairs
mkdir -p MicroC_output/aggregate_profiles
mkdir -p MicroC_output/individual_viewpoint_profiles
mkdir -p MicroC_output/cools
...
for rep in ${reps[@]}
do
	echo "downloading Micro-C ${!rep} fastq files" >&3
	...
	echo "mapping and parsing Micro-C ${!rep}" >&3
	bwa mem -SP5M -t 16 Auxiliary_data/hg19/male.hg19.fa tmp/MicroC_${!rep}_1.fastq.gz tmp/MicroC_${!rep}_2.fastq.gz | pigz -p 16 > tmp/MicroC_${!rep}.sam.gz
	rm tmp/*.fastq.gz
	pairtools parse --cmd-in 'pigz -d -p 16' --cmd-out 'pigz -p 16' -c Auxiliary_data/hg19/hg19.chrom.sizes --output-stats MicroC_output/mapping_stats/stats_map_MicroC_${!rep}.txt -o tmp/MicroC_${!rep}.pairsam.gz tmp/MicroC_${!rep}.sam.gz
	rm tmp/*.sam.gz
	...
	echo "deduplicating and selecting viepoint reads in Micro-C ${!rep}" >&3
...
echo "loading mcool Micro-C file" >&3
wget https://zenodo.org/records/12791031/files/K562_MicroC.hg19.mcool -q -P MicroC_output/cools/
echo "extracting promoter-relevant data from the genome-wide matrix" >&3
cooler dump --join -b MicroC_output/cools/K562_MicroC.hg19.mcool::/resolutions/250 | awk '($1 == $4)' | awk '$8=="" {$8="0"} 1' OFS="\t" | pairToBed -a stdin -b MChIPC_output/mononucleosomal_and_ChIP/binned_peaks.bed > tmp/MicroC_filtered.bedpe
./Helpers/processing_MicroC_helper_1.R
echo "generating aggregate ICE Micro-C profiles" >&3
bedGraphToBigWig tmp/MicroC.ICE.bedgraph Auxiliary_data/hg19/hg19.chrom.sizes tmp/MicroC.ICE.bw
...
echo "calling Micro-C point interactions" >&3

for res in 250 500 1kb 2kb 5kb 10kb
do
	mustache -f MicroC_output/cools/K562_MicroC.hg19.mcool -r $res -pt 0.1 -st 0.88 -oc 2 -p 16 -v 0 -o tmp/MicroC_loops_$res.tsv
done
./Helpers/processing_MicroC_helper_2.R

echo "generating Micro-C signal file for plotting E(DHS)-P heatmaps" >&3
...
echo "generating Micro-C profiles for individual viewpoints" >&3
