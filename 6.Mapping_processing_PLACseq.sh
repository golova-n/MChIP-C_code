...
mkdir -p PLACseq_output/mapping_stats
mkdir -p PLACseq_output/sam
mkdir -p PLACseq_output/pairs
mkdir -p PLACseq_output/aggregate_profiles
mkdir -p PLACseq_output/individual_viewpoint_profiles

echo "downloading PLAC-seq fastq files" >&3
...
echo "mapping and parsing PLAC-seq rep1 data" >&3
bwa mem -SP5M -t 16 Auxiliary_data/hg19/male.hg19.fa tmp/4DNFIM5BWS5H.fastq.gz tmp/4DNFITMUHEBY.fastq.gz | pigz -p 16 > tmp/PLACseq_rep1_1.sam.gz
...
bwa mem -SP5M -t 16 Auxiliary_data/hg19/male.hg19.fa tmp/4DNFI9TNKOBD.fastq.gz tmp/4DNFI2I5A8ZU.fastq.gz | pigz -p 16 > tmp/PLACseq_rep1_2.sam.gz
...
bwa mem -SP5M -t 16 Auxiliary_data/hg19/male.hg19.fa tmp/4DNFIZEP644W.fastq.gz tmp/4DNFIMWVLMTQ.fastq.gz | pigz -p 16 > tmp/PLACseq_rep1_3.sam.gz
...
echo "mapping and parsing PLAC-seq rep2 data" >&3
bwa mem -SP5M -t 16 Auxiliary_data/hg19/male.hg19.fa tmp/4DNFIP2TS3O8.fastq.gz tmp/4DNFIU98V4FB.fastq.gz | pigz -p 16 > tmp/PLACseq_rep2_1.sam.gz
...
bwa mem -SP5M -t 16 Auxiliary_data/hg19/male.hg19.fa tmp/4DNFI69HRUMH.fastq.gz tmp/4DNFIMTJX2X7.fastq.gz | pigz -p 16 > tmp/PLACseq_rep2_2.sam.gz
...
bwa mem -SP5M -t 16 Auxiliary_data/hg19/male.hg19.fa tmp/4DNFICGRXFU6.fastq.gz tmp/4DNFIJYF2QVF.fastq.gz | pigz -p 16 > tmp/PLACseq_rep2_3.sam.gz
...
do
	echo -e "\tmerging and deduplicating and selecting viewpoint signal in $rep PLAC-seq" >&3
...
echo "merging viewpoint signals of PLAC-seq replicates" >&3
...
echo "generating aggregate PLAC-seq profiles" >&3
...
echo "loading PLAC-seq interaction files and merging them" >&3
...
echo "generating PLAC-seq signal file for plotting E(DHS)-P heatmaps" >&3
