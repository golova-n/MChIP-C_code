...
wget https://storage.googleapis.com/encode-pipeline-genome-data/hg19/bwa_index/male.hg19.fa.tar -q -P tmp/hg19
tar -xf tmp/hg19/male.hg19.fa.tar -C tmp/hg19/
rm tmp/hg19/male.hg19.fa.tar
wget https://storage.googleapis.com/encode-pipeline-genome-data/hg19/hg19.chrom.sizes -q -P tmp/hg19
wget https://hgdownload.soe.ucsc.edu/goldenPath/hg38/liftOver/hg38ToHg19.over.chain.gz -q -P tmp/hg19

echo "downloading MChIPC_rep1_1 fastq files"
wget -q -P tmp/ https://sra-pub-run-odp.s3.amazonaws.com/sra/SRR23410275/SRR23410275
fasterq-dump -e 16 -O tmp/ tmp/SRR23410275
rm tmp/SRR23410275
pigz -p 16 tmp/*.fastq

echo "mapping MChIPC_rep1_1"
bwa mem -SP5M -t 16 tmp/hg19/male.hg19.fa tmp/SRR23410275_1.fastq.gz tmp/SRR23410275_2.fastq.gz 2>/dev/null | pigz -p 16 > tmp/MChIPC_rep1_1.sam.gz
rm tmp/*.fastq.gz

echo "downloading MChIPC_rep1_2 fastq files"
wget -q -P tmp/ https://sra-pub-run-odp.s3.amazonaws.com/sra/SRR23410276/SRR23410276
fasterq-dump -e 16 -O tmp/ tmp/SRR23410276 
rm tmp/SRR23410276
pigz -p 16 tmp/*.fastq

echo "mapping MChIPC_rep1_2"
bwa mem -SP5M -t 16 tmp/hg19/male.hg19.fa tmp/SRR23410276_1.fastq.gz tmp/SRR23410276_2.fastq.gz 2>/dev/null | pigz -p 16 > tmp/MChIPC_rep1_2.sam.gz
rm tmp/*.fastq.gz

echo "downloading MChIPC_rep1_3 fastq files"
wget -q -P tmp/ https://sra-pub-run-odp.s3.amazonaws.com/sra/SRR23410274/SRR23410274
fasterq-dump -e 16 -O tmp/ tmp/SRR23410274 
rm tmp/SRR23410274
pigz -p 16 tmp/*.fastq

echo "mapping MChIPC_rep1_3"
bwa mem -SP5M -t 16 tmp/hg19/male.hg19.fa tmp/SRR23410274_1.fastq.gz tmp/SRR23410274_2.fastq.gz 2>/dev/null | pigz -p 16 > tmp/MChIPC_rep1_3.sam.gz
rm tmp/*.fastq.gz

for rep in rep1_1 rep1_2 rep1_3;
do
	echo "parsing MChIPC_$rep data";
	pairtools parse --cmd-in 'pigz -d -p 16' --cmd-out 'pigz -p 16' -c tmp/hg19/hg19.chrom.sizes --output-stats MChIPC_output/mapping_stats/stats_map_MChIPC_$rep.txt -o tmp/MChIPC_$rep.pairsam.gz tmp/MChIPC_$rep.sam.gz;
	...
	echo "sampling from MChIPC_$rep data";
	for sample in 0.1 0.25 0.5;
	do 
		echo "sampling $sample proportion of reads";
...
echo "merging technical replicates for MChIPC_rep1 mononucleosomal signal"
...
echo "merging technical replicates for MChIPC_rep1 MChIP-C signal"
...
for rep in rep1;
do
	echo "primary processing of $rep data";
	echo "deduplicating mononucleosomal signal from full dataset";
	...
	echo "deduplicating MChIP-C signal";
...
echo "primary processing of downsampled datasets"
for sample in 0.1 0.25 0.5;
do
	echo "processing $sample (downsampled) datasets";
	for rep in rep1;
	do
		eval "nreads_$rep=$(gunzip -c tmp/MN_$rep.sub_$sample.pairsam.gz | wc -l)";
	done
	max=$(printf "%s\n" $nreads_rep1 | sort -n | head -1)
	if [ $max -gt 20000000 ]; 
	then
		max=20000000; 
	fi
