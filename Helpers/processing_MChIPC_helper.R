...
vp <- read.csv("MChIPC_output/mononucleosomal_and_ChIP/binned_peaks.bed", header = F, sep = "\t")
...
vp <- vp[,c(1:4,9)]%>% group_by(queryHits) %>% mutate(genes=paste0(gene, collapse=","))
vp <- vp[,c(1:3,6)] %>% distinct()
for (i in 1:nrow(vp)){
  if (vp[i,4]=="NA"){
