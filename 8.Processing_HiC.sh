...
echo "downloading mcool Hi-C file" >&3
wget https://zenodo.org/records/12791031/files/K562_HiC.hg19.mcool -q -P HiC_output/cools/
echo "extracting promoter-relevant data from the genome-wide matrix" >&3
cooler dump --join -b HiC_output/cools/K562_HiC.hg19.mcool::/resolutions/1000 | awk '($1 == $4)' | awk '$8=="" {$8="0"} 1' OFS="\t" | pairToBed -a stdin -b MChIPC_output/mononucleosomal_and_ChIP/binned_peaks.bed > tmp/HiC_filtered.bedpe
./Helpers/processing_HiC_helper_1.R
echo "generating aggregate Hi-C profiles" >&3
...
echo "generating aggregate ICE Hi-C profiles" >&3
...
echo "calling Hi-C point interactions" >&3
for res in 1kb 2kb 5kb 10kb 20kb
do
	mustache -f HiC_output/cools/K562_HiC.hg19.mcool -r $res -pt 0.1 -st 0.88 -oc 2 -p 16 -v 0 -o tmp/HiC_loops_$res.tsv
done
./Helpers/processing_HiC_helper_2.R
echo "generating Hi-C signal file for plotting E(DHS)-P heatmaps" >&3
